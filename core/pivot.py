...
def find_pivots(strokes: Sequence[Stroke], min_strokes: int = 3) -> List[Pivot]:
    ...
    while i + min_strokes - 1 < n:
        ...
        while j < n:
            candidate = strokes[j]
            # 延伸必须同时满足：笔与中枢区间有价格重叠，且终点仍在区间内。
            # 仅看终点会把大幅离开笔错误吞入中枢，导致区间失真。
            overlaps = candidate.low <= zg and candidate.high >= zd
            ends_inside = zd <= candidate.end.price <= zg
            if not (overlaps and ends_inside):
                break
